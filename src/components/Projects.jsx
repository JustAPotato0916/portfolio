...
const projectsInfo = [
  {
    name: 'Portfolio',
    description:
      '這個專案使用 React 作為前端框架，搭配 Tailwind CSS 進行樣式設計，並利用 GSAP 創建動態且引人入勝的動畫效果。這個組合使得個人網站變得視覺上令人驚艷，並提供流暢的用戶體驗。透過 React 的強大功能、Tailwind CSS 的統一設計風格，以及 GSAP 的滾動觸發功能和動畫工具，實現了動態導覽列、滾動觸發動畫和平滑滾動等功能，打造出一個模組化、可維護且在不同設備上都能呈現最佳效果的個人網站。',
  ...
  },
  {
    name: 'ChatPDF',
    description:
      '這個專案整合了多項現代化技術以實現完整的用戶服務流程：透過 Clerk 提供無縫的第三方身份驗證（如 Google、GitHub）與註冊登入功能，搭配 Stripe 訂閱系統管理付費方案與支付流程，並利用 AWS S3 安全儲存用戶上傳的 PDF 文件。此外，系統內建智能聊天室，透過 OpenAI API 分析文件內容，提供即時且精準的問答互動，實現從身份驗證、付費訂閱到文件管理的全流程自動化，同時強化用戶體驗與數據安全性',
  ...
  },
  {
    ...
    description:
      '這個專案使用 React 作為前端框架，打造一個互動性強的留言版，允許用戶發布和回覆留言。後端則採用 Fastify 來構建高效的 API 伺服器，並利用 Prisma 進行資料模型管理，連接 PostgreSQL 資料庫以儲存和管理留言數據。這個技術堆疊使得系統能夠提供快速、可靠且易於維護的留言功能，同時確保數據一致性和安全性。',
...
const Projects = () => {
  ...
  const changeCardIndex = (direction) => {
    if (direction === 'next') {
      setCardIndex(
        (prevIndex) =>
          (prevIndex = prevIndex === totalProjects - 1 ? 0 : prevIndex + 1)
      );
    } else if (direction === 'prev') {
      setCardIndex(
        (prevIndex) =>
          (prevIndex = prevIndex === 0 ? totalProjects - 1 : prevIndex - 1)
      );
    }
  };
